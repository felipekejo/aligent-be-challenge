export function differenceDates(
  ...
  switch (unit) {
    ...
    case 'years':{
      ...
      const yearDiff = Math.abs(new Date(secondDate).getUTCFullYear() - new Date(firstDate).getUTCFullYear());

      // Check if we need to adjust the year difference by comparing month and day
      const isEndDateEarlierInYear = 
        new Date(secondDate).getUTCMonth() <  new Date(firstDate).getUTCMonth() ||
        (new Date(secondDate).getUTCMonth() ===  new Date(firstDate).getUTCMonth() &&
